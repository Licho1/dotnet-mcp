...
| `find-references` | Find all usages of a symbol across the solution (semantic, not text search). Maps Razor `.g.cs` locations back to original `.cshtml` files. Also accepts `filePath:line:col` for both `.cs` and `.cshtml` files. |
